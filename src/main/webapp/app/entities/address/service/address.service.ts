...
type RestOf<T extends IAddress | NewAddress> = Omit<T, 'lastModified'> & {
...
export class AddressService {
  ...
  protected convertDateFromClient<T extends IAddress | NewAddress | PartialUpdateAddress>(address: T): RestOf<T> {
    return {
      ...address,
      lastModified: address.lastModified?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAddress: RestAddress): IAddress {
    return {
      ...restAddress,
      lastModified: restAddress.lastModified ? dayjs(restAddress.lastModified) : undefined,
    };
  }
