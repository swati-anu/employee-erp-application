...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'joindate' | 'lastModified'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      joindate: employee.joindate?.toJSON() ?? null,
      lastModified: employee.lastModified?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      joindate: restEmployee.joindate ? dayjs(restEmployee.joindate) : undefined,
      lastModified: restEmployee.lastModified ? dayjs(restEmployee.lastModified) : undefined,
    };
  }
